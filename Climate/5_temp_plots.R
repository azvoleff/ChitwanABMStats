...
temp$Season <- NA
temp$Season[temp$Month %in% c(6, 7, 8, 9)] <- 'Monsoon'
temp$Season[temp$Month %in% c(10, 11, 12, 1)] <- 'Winter'
temp$Season[temp$Month %in% c(2, 3, 4, 5)] <- 'Spring'
...
seasonal_diurnal_plot <- ggplot(seasonal_stats, aes(season_start_year, 
                                                    ...
                                                    linetype=Season)) +
    geom_line() + xlab('Time') +
    ylab('Mean daily diurnal temperature range') + 
    geom_smooth(method="lm", se=TRUE)
...
seasonal_melt_plot <- ggplot(seasonal_melt, aes(season_start_year, value, 
                                                colour=Season)) +
    geom_line() + xlab('Time') + facet_grid(. ~ variable) +
    ylab('Temperature (C)') + 
    geom_smooth(method="lm", se=TRUE)
